...
@section('content')
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('dashboard.category_articles.index') }}">مقالات
                                الاقسام</a></li>
                        <li class="breadcrumb-item active">أضافة مقال في قسم</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <form action="{{ route('dashboard.category_articles.store') }}" class="row" method="post"
                        ...
                        @foreach ($item->translations as $t)
                            @if ($t->locale == 'ar')
                                <option value="{{ $item->id }}"
                                    {{ old('category') === $item->id ? 'selected' : null }}>{{ $t->name }}</option>
                            @endif
                        @endforeach
                    @endforeach
                </select>
